...
def time_methods() -> Tuple[List[float], List[float]]:
    ...
    for size in SIZES:
        # 1. Initialize the sample queues
        stacks = _setup_stacks(size, NUM_TRIALS)

        # 2. For each stack created, call the function timeit.
        #    timeit takes three arguments:
        #        - a *string* representation of a piece of code to run
        #        - the number of times to run it (just 1 for us)
        #        - globals is a technical argument that you DON'T need to
        ...
        time = 0
        for stack in stacks:
            time += timeit('stack.push(1)', number=1, globals=locals())
        ...
        # 4. Report the average time taken and add that to our list of
        #    results.
        stack_times.append(average_time)
        print(f'push: Stack size {size:>7}, time: {average_time}')

    # Do not change the return statement below.
    return queue_times, stack_times


def time_methods_popper() -> Tuple[List[float], List[float]]:
    """Run timing experiments for enqueue on LinkedListQueue and
    push for LinkedListStack, returning lists with the average time it took to
    enqueue a single element to queues with sizes QUEUE_SIZES over
    NUM_TRIALS trials."""
    ...
    # This loop runs the timing experiment for enqueueing one item to
    # LinkedListQueue.
    print("Running LinkedListQueue.dequeue experiments...")
    for size in SIZES:
        # 1. Initialize the sample queues
        queues = _setup_queues(size, NUM_TRIALS)
        ...
        # 4. Report the average time taken and add that to our list of
        #    results.
        queue_times_dequeue.append(average_time)
        print(f'dequeue: Queue size {size:>7}, time: {average_time}')

    # This loop runs the timing experiment for enqueueing one item to
    # LinkedListQueue.
    print("Running LinkedListStack.pop experiments...")
    for size in SIZES:
        # 1. Initialize the sample queues
        stacks = _setup_stacks(size, NUM_TRIALS)
        ...
        time = 0
        for stack in stacks:
            time += timeit('stack.pop', number=1, globals=locals())
        ...
        # 4. Report the average time taken and add that to our list of
        #    results.
        stack_times_pop.append(average_time)
        print(f'push: Stack size {size:>7}, time: {average_time}')

    # Do not change the return statement below.
    return queue_times_dequeue, stack_times_pop
...
def plot_experiment() -> None:
    ...
    queue_times_dequeue, stack_times_pop = time_methods_popper()
