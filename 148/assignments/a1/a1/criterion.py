        ...
        for i, ans in enumerate(answers[:-1]):
            for ans2 in answers[i + 1:]:
                if not ans.is_valid(question):
                    raise InvalidAnswerError
                if not ans2.is_valid(question):
                    raise InvalidAnswerError

                total += question.get_similarity(ans, ans2)
                num_comp += 1

        return total / num_comp
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        for answer in answers:
            if not answer.is_valid(question):
                raise InvalidAnswerError
            answer_content.append(answer.content)

        for ans in answer_content:
            if answer_content.count(ans) == 1:
                return 0.0
        return 1.0
