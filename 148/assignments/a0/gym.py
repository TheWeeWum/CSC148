...
class Instructor:
    """Class for the instructor of the gym

    === Public Attributes ===
    name: The name of the instructor.

    === Private Attributes ===
    _id: unique id for the instructor
    _certifications: list of instructors certifications
    """
    ...
    def __init__(self, idd: int, instructors_name: str) -> None:
        self.name = instructors_name
        self._id = idd
    ...
    def get_certificates(self) -> list[str]:
        """returns the list of certificates for the instructor"""
        return self._certifications[:]

    def add_certificate(self, cert: str) -> bool:
        """
        adds a certificate to the list of certificates
        which the instructor currently possesses.
        """
    ...
    def is_qualified(self, class_name: str) -> bool:
        """
        returns a boolean value of whether the instructor
        is qualified to teach the class or not
        """
        return class_name in self._certifications
...
class Gym:
    ...
    def _instructor_in_room_helper(
            self, rooms: dict[str, int], instructor: Instructor) -> bool:
        for info in rooms.values():
            if instructor == info[0]:
                return False
        return True
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        r_n_avail = True
        instruct_avail = True
        for time, rooms in self._schedule.items():
            if time == time_point:
                r_n_avail = room_name not in rooms
                instruct_avail = self._instructor_in_room_helper(
                    rooms, instructor)

        # check if instructor is qualified
        workout_class = self._workouts[workout_name]
        instruct_qualified = True
        for cert in workout_class.get_required_certificates():
            if cert not in instructor.get_certificates():
                instruct_qualified = False

        if r_n_avail and instruct_avail and instruct_qualified:
            if time_point not in self._schedule:
                self._schedule[time_point] = {}
            # for time, rooms in self._schedule:
            #     if time == time_point:

            self._schedule[time_point][room_name] = \
                (instructor, workout_class, [])
            return True
        else:
            return False
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        for time, rooms in self._schedule.items():
            for room_name, info in rooms.items():
                if client in info[2]:
                    return False
                if workout_name == info[1].name and \
                        time == time_point and len(info[2]) > emptiest_num:
                    emptiest_class = room_name
                    emptiest_num = len(info[2])

        self._schedule[time_point][emptiest_class][2].append(client)
        return True
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        True
        """
        dic = {}  # instructor_id: hours worked
        for inst in self._instructors:
            dic[inst] = 0
        for time in self._schedule:
            if time1 <= time <= time2:
                for room in self._schedule[time].values():
                    inst_id = room[0].get_id()
                    dic[inst_id] += 1
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        list_of_pay = []  # tup(id, name, hours worked, total pay)
        hours_per_instruct = self.instructor_hours(time1, time2)
        for idd, hours in hours_per_instruct.items():
            rate = base_rate + 1.5 * len(
                self._instructors[idd].get_certificates())
            list_of_pay.append(
                (idd, self._instructors[idd].name, hours, hours * rate))
        return list_of_pay
    ...
    def _is_instructor_name_unique(self, instructor: Instructor) -> bool:
        ...
        """
        counter = 0
        for instruct in self._instructors.values():
            if instructor.name == instruct.name:
                counter += 1
        if counter <= 1:
            return True
        else:
    ...
    def _offerings_at_helper(self, course: dict[str, tuple[
            Instructor, WorkoutClass, list[str]]]) -> bool:
        """
        Helper function fo the offerings at method
        just handles the appending as well as one of the for loops
        and uniqueness of the instructor name.
        Added to avoid to many layers
        """
        info = []
        for room_name, i in course.items():
            cap = self._room_capacities[room_name]
            if self._is_instructor_name_unique(i[0]):
                info.append((i[1].name, room_name, len(i[2]),
                             cap - len(i[2]), i[0].name, room_name))
            else:
                info.append((i[1].name, room_name, len(i[2]),
                             cap - len(i[2]), i[0].name + ' (' + str(
                             i[0].get_id()) + ')', room_name))
        return info
    ...
    def offerings_at(self, time_point: datetime) -> list[dict[str, str | int]]:
        ...
        {'Date': 'Friday, 2022-09-09', 'Time': '12:00',
         ...
         'Available': 50, 'Instructor': 'Diane (1)'}
         """
        date = time_point.strftime("%A, %Y-%m-%d")  # 'Monday, 2023-01-22'
        time = str(time_point.time())[:5]
        # [(class_name, room_name, registered, available, instructor)]
        info = []
        # room_name: tuple[Instructor, WorkoutClass, list[str]]
        for course_time, course in self._schedule.items():
            if course_time == time_point:
                info = self._offerings_at_helper(course)
        dic = {}
        offerings = []
        for tup in info:
            dic[tup[5]] = tup[:-1]
        new_dic = dict(sorted(dic.items()))
        for tup in new_dic.values():
            offerings.append(
                create_offering_dict(date, time, tup[0], tup[1],
                                     tup[2], tup[3], tup[4]))
        return offerings
    ...
    def to_schedule_list(self, week: datetime = None) \
        ...
        if week:
            for each in keys:
                if in_week(each, week):
                    new_keys.append(each)
        else:
            new_keys = keys

        for all_times in new_keys:
            for each in self.offerings_at(all_times):
                lst.append(each)
        return lst
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        True
        """
        sched, work, inst, room = other._get_info()
        if self._schedule != sched or self._workouts != work:
            return False
        if self._instructors != inst or self._room_capacities != room:
            return False
        else:
            return True

    def _get_info(self) -> \
            tuple[dict[datetime, dict[str,
                  tuple[Instructor, WorkoutClass, list[str]]]],
                  dict[str, WorkoutClass],
                  dict[int, Instructor],
                  dict[str, int]]:
        """
        returns various attributes of the Gym class
        implemented to simplify one of the methods.
        """
        return self._schedule, \
            self._workouts, self._instructors, self._room_capacities
...
def gym_from_yaml(filename: str) -> Gym:
    ...
    for event in gym_data['schedule']:
        ...
        for participant in event['participants']:
            gym.register(
                event['time'], participant, event['workout_class'])
