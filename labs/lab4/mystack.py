...
def double_stack(s: Stack) -> Stack:
    ...
    new_stack = Stack()
    while not s.is_empty():
        # populate a new upside down stack of side_stack
        # to reconstruct the initial stack.
        # Then add each element twice to a new stack
        x = s.pop()
        side_stack.push(x)
        new_stack.push(x)
        new_stack.push(x)
    ...
    # maintains the order of the new_stack if this is desired
    newer_stack = Stack()
    while not new_stack.is_empty():
        newer_stack.push(new_stack.pop())
    return newer_stack
    # return new_stack
