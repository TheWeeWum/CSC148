...
class Group:
    ...
    def __contains__(self, member: Student) -> bool:
        ...
        as <member>.
        """
        if member.id in [x.id for x in self._members]:
            return True
        return False
    ...
    def __str__(self) -> str:
        ...
        # should never occur as group must contain
        # at least 1 student
        if len(self._members) == 0:
            return string
        elif len(self._members) == 1:
            return self._members[0].name
        else:
            string = self._members[0].name
...
class Grouping:
    ...
    def __str__(self) -> str:
        ...
        string = self._groups[0].__str__()
        for each in self._groups[1:]:
            string += "\n" + each.__str__()
        return string
    ...
    def add_group(self, group: Group) -> bool:
        ...
        """
        # group should never be empty, but incase it is
        if len(group) == 0:
            return False

        temp_ids = []
        for student in group.get_members():
            temp_ids.append(student.id)
        for ids in temp_ids:
            if temp_ids.count(ids) > 1:
                return False

        for in_group in self._groups:
            for student in group.get_members():
                # if the exact same student is in
                if student.id in [x.id for x in in_group.get_members()]:
                    return False
    ...
    def get_groups(self) -> list[Group]:
        ...
        See the handout for more information about what a shallow copy is.
        """
        # NOT A DOUBLE SHALLOW
        # just a single shallow, like specified in docstring
        return self._groups[:]
...
class Grouper:
    ...
    def __init__(self, group_size: int) -> None:
        ...
        # should never happen
        if group_size <= 1:
            raise ValueError("Group size too small")
...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        # not technically necessary, doesn't actually do anything
        # if the number of students is always greater than min group size
...
class GreedyGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        grouping = Grouping()
        unused_students = sort_students(list(course.get_students()), 'id')
        temp_group = []
        while unused_students:
            if not temp_group:
                temp_group = [unused_students[0]]
                unused_students.pop(0)
            if len(temp_group) == self.group_size:
                grouping.add_group(Group(temp_group))
                temp_group = []
            if len(unused_students) == 0:
                grouping.add_group(Group(temp_group))
                break
            ...
            #     sent_group = temp_group + [student]
            #     score = survey.score_students(sent_group)
            #     if score >= best_score:
            #         if not best_student or student.id > best_student.id:
            #             best_student = student

            # accidentally coded above before realizing it's already coded.
            best_student = find_best_addition_to_group(
                survey, temp_group, unused_students)

            temp_group.append(best_student)
            unused_students.remove(best_student)
...
class SimulatedAnnealingGrouper(Grouper):
    ...
    === Private Attributes ===
    _iterations: The number of iterations to be run on the algorithm
    _initial_temperature: given temperature, determines some sort of ramdomness?

    === Representation Invariants ===
    group_size > 1
    iterations > 1, also works if iterations = 0, but would normally raise error
    if iterations = 1, however I implemented a case for that, but I'm not sure
    if it should do what I programmed it to do
    initial_tempurature >= 0
    """
    group_size: int
    _iterations: int
    ...
    def __init__(self,
        ...
        """
        if iterations <= 1:
            raise ValueError("Iterations can not be less than 1")
        if group_size <= 1:
            raise ValueError("Group size must be more than 1")
        if initial_temperature < 0:
            raise ValueError("Temperature mustn't be negative")

        Grouper.__init__(self, group_size)
        self._iterations = iterations
        self._initial_temperature = initial_temperature

    def _make_grouping_helper(self, current_list_of_groups: list[list[Student]],
                              survey: Survey) -> list[list[Student]]:
        """Helper for the make_grouper method
        Calculates the best_list_of_groups add to the grouping
        Return a list containing lists of students.
        <current_list_of_groups> represents the current groups that
        we are going to add to the grouping
        <survey> is a survey containing the questions the students will be
        tested on
        """
        best_list_of_groups = current_list_of_groups
        old_score = total_score(survey, current_list_of_groups)
        best_score = float('-inf')
        last_list = deepcopy(current_list_of_groups)

        for i in range(self._iterations):
            random_swap(current_list_of_groups, i)
            score = total_score(survey, current_list_of_groups)
            if self._iterations == 1:
                t = self._initial_temperature
            else:
                t = self._initial_temperature * (1 - i / (self._iterations - 1))

            if score > best_score:
                best_score = score
                old_score = score
                best_list_of_groups = deepcopy(current_list_of_groups)
                last_list = deepcopy(current_list_of_groups)

            elif accept(old_score, score, t, i):
                old_score = score
                last_list = deepcopy(current_list_of_groups)

            else:
                current_list_of_groups = deepcopy(last_list)

        return best_list_of_groups
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        if len(course.get_students()) <= self.group_size:
            raise ValueError("Course must have more students than the "
                             "group size")

        l_of_stu = sort_students(list(course.get_students()), 'id')
        # l_of_stu = list(course.get_students())
        current_list_of_groups = slice_list(l_of_stu, self.group_size)
        ...
        for stu in l_of_stu:
            for question in survey.get_questions():
                if stu.get_answer(question) is None:
                    raise ZeroDivisionError

        if len(survey.get_questions()) == 0 or len(current_list_of_groups) == 1:
            grouping = Grouping()
            for group in current_list_of_groups:
                grouping.add_group(Group(group))

            return grouping

        best_list_of_groups = self._make_grouping_helper(
            current_list_of_groups, survey)

        grouping = Grouping()
        for group in best_list_of_groups:
            grouping.add_group(Group(group))
